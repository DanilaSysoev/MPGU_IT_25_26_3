...
| **Чистота репозитория** | **+2** | `.gitignore` работает корректно. В репозитории **нет** папки `venv`, `__pycache` и файлов IDE. *(Если мусор есть — 0 баллов за этот пункт + штраф).* |
| **История коммитов (Git Flow)** | **+3** | **Ключевой пункт.** История коммитов показывает, что разработка велась итеративно. Есть минимум 2 коммита. В ветке `main` присутствует файл `main.py` и обновленный `README.md` (доказательство успешного `merge`). |
