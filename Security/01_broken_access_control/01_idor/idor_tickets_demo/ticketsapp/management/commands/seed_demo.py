...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        if not Ticket.objects.exists():
            ...
            Ticket.objects.create(owner=mod, title='Mod Ticket X')
        from ...models import Message
        if not Message.objects.exists():
            Message.objects.create(owner=dev, text='Dev Message A')
            Message.objects.create(owner=mod, text='Mod Message X')
